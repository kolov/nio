...
public class NioServer implements Runnable {
    ...
    public void send(SocketChannel socketChannel, byte[] data) {
        synchronized (this.pendingChanges) {
            ...
            System.out.println("Queueing OP_WRITE change request for socketChannel " + System.identityHashCode(socketChannel));
                        ...
                        switch (change.type) {
                            case ChangeRequest.CHANGEOPS:
                                ...
                                try {
                                    ...
                                    System.out.println("error changing socketChanneh " + System.identityHashCode(change.socket) +
                                            " to " + change.ops);
                                }
                        }
                    }
    ...
    private int connectioNo = 0;
    private void accept(SelectionKey key) throws IOException {
        System.out.println("Accept connection #" + ++connectioNo);
    ...
    private Map<SocketChannel, ServerResponse> responses = new HashMap<SocketChannel, ServerResponse>();

    public ServerResponse getResponse(SocketChannel c) {
        synchronized (responses) {
            ServerResponse result = responses.get(c);
            if (result == null) {
                result = new ServerResponse();
                responses.put(c, result);
            }
            return result;
        }
    }

    public void dismissResponse(SocketChannel socket) {
        synchronized (responses) {
            responses.remove(socket);
        }
    }
}